...
/**
 * Returns the success value or throws the task's error when the result is a failure.
 *
 * @param result - A single {@link TaskResult}.
 * @returns The success `value` when `result.ok` is true.
 * @throws The `error` from the result when `result.ok` is false.
 */
